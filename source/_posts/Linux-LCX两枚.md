...
int lcx_slave(unsigned long ip1,unsigned short port1,unsigned long ip2,unsigned short port2)
{
    SOCKET_INIT
    struct sockaddr_in sa[2];
    SOCKET s[2];
...
int main_func(int argc,char**argv)
{
    ...
    int32_t n = 2;
    int32_t addr1=0,addr2=0,port1=0,port2=0;
    switch (i)
    {
    case 0:
        about();
